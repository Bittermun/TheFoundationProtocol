...
class ThresholdReleaser:
    ...
    def contribute_signature(
        ...
        if len(release.signatures_collected) >= release.required_signatures:
            ...
            return True, f"Threshold reached! Release complete ({len(release.signatures_collected)}/{release.required_signatures})"
        
        remaining = release.required_signatures - len(release.signatures_collected)
        return True, f"Signature accepted ({len(release.signatures_collected)}/{release.required_signatures}, {remaining} remaining)"
