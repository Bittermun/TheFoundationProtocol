...
| L4 | **SQLite is not safe for `--workers > 1`** | Concurrent writes can corrupt the database under multi-worker uvicorn. | Documented in the deployment runbook. Use `--workers 1` or migrate to PostgreSQL for horizontal scaling. |
