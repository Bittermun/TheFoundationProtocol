...
|---|-----------|--------|-----------|--------|
| L1 | ~~**In-memory rate limiters**~~ — Redis-backed distributed rate limiting is now implemented via `_RedisRateLimiterAdapter`. | N/A — set `TFP_REDIS_URL` for distributed deployments. | Redis sliding-window counters with Lua scripts provide atomic, shared state across workers. Falls back to in-memory if Redis unavailable. | ✅ **RESOLVED** (v3.1+) |
| L2 | ~~**Broad exception swallowing in auto-mint**~~ — The referenced `server.py:1431` no longer contains auto-mint code; this was outdated documentation. | N/A — `SupplyCapError` propagates correctly from `TaskStore.increment_total_minted()`. | No action required; supply cap enforcement is tested in `tests/test_supply_cap.py`. | ✅ **OUTDATED** — Issue did not exist |
