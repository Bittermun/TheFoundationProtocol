...
class BloomFilter:
    ...
    def _hashes(self, item: bytes) -> List[int]:
        ...
        h1 = int.from_bytes(
            hashlib.sha3_256(item + self.seed.to_bytes(4, 'big')).digest()[:8],
            'big'
        )
        h2 = int.from_bytes(
    ...
    def deserialize(cls, data: bytes) -> 'BloomFilter':
        ...
        if len(bf.bit_array) != expected_byte_size:
            raise ValueError(f"Byte array size mismatch: expected {expected_byte_size}, got {len(bf.bit_array)}")
    ...
    def union(self, other: 'BloomFilter') -> 'BloomFilter':
        ...
        """
        if (self.size_bits != other.size_bits or 
