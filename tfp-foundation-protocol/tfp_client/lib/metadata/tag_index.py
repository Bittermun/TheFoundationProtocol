...
class TagOverlayIndex:
    ...
    def add_entry(self, domain: str, tags: List[str], content_hash: bytes, 
    ...
    def _build_merkle_root(self, entries: List[TagEntry]) -> bytes:
        ...
        for entry in entries:
            ...
            entry_data = f"{entry.tag}:{entry.content_hash.hex()}:{entry.popularity_score}"
    ...
    def get_merkle_proof(self, dag: TagIndexDAG, tag: str, 
                         content_hash: bytes) -> Optional[List[bytes]]:
        """
    ...
    def verify_merkle_proof(self, leaf_data: str, proof: List[Tuple[str, bytes]], 
    ...
    def get_popular_entries(self, dag: TagIndexDAG, min_popularity: float = 0.8) -> List[TagEntry]:
        """
    ...
    def get_stats(self, domain: str, epoch: int) -> dict:
        ...
        avg_pop = sum(e.popularity_score for e in entries) / len(entries) if entries else 0.0
