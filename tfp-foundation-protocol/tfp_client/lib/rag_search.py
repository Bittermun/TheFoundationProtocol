...
class RAGGraph:
        ...
        for i, line in enumerate(lines):
            ...
            if current_token_count + line_tokens > self.chunk_size:
                ...
                current_token_count = sum(
                    len(l) // 4 + 1 for l in current_chunk_lines
                )
    ...
    def index_directory(
        ...
        if exclude_dirs is None:
            exclude_dirs = ["__pycache__", ".git", "node_modules", ".pytest_cache", "venv"]
        ...
        logger.info(
            f"Indexed {files_indexed} files ({total_chunks} chunks) "
            f"from {directory}"
        )
        return total_chunks
