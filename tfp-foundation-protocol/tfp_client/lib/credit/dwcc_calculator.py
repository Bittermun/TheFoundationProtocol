...
class DWCCEntry:
    ...
    content_hash: str
    requests: int = 0
    first_seen: float = dataclasses.field(default_factory=lambda: datetime.now(timezone.utc).timestamp())
    last_requested: float = dataclasses.field(default_factory=lambda: datetime.now(timezone.utc).timestamp())
    pinned: bool = True
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DWCCEntry":
        ...
        return cls(
            ...
            last_requested=data.get("last_requested", datetime.now(timezone.utc).timestamp()),
...
class DWCCCalculator:
    ...
    def track_request(self, content_hash: str, semantic_tier: Optional[SemanticValueTier] = None) -> None:
        """
    ...
    def calculate_dwcc(self, content_hash: str, current_time: Optional[float] = None) -> float:
        """
