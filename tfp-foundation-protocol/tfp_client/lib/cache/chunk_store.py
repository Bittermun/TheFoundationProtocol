...
class ChunkStore:
    ...
    def put(
        ...
        with self._lock:
            ...
            if chunk_id in self._chunks:
                ...
                self._total_bytes -= old_entry.size_bytes if hasattr(old_entry, 'size_bytes') else len(old_entry.data)
    ...
    def clear(self) -> None:
        ...
        with self._lock:
            ...
            bloom_hashes = BloomFilter.optimal_hash_count(m=bloom_size, n=self._max_chunks)
            self._bloom = BloomFilter(size_bits=bloom_size, hash_count=bloom_hashes)
    ...
    def get_by_category(self, category: str) -> List[ChunkCacheEntry]:
        ...
        with self._lock:
            return [
                entry for entry in self._chunks.values()
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                'utilization_chunks': len(self._chunks) / self._max_chunks if self._max_chunks > 0 else 0,
                'utilization_bytes': self._total_bytes / self._max_bytes if self._max_bytes > 0 else 0,
            }
