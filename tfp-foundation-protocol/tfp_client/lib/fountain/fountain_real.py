...
class RealRaptorQAdapter:
    ...
    def encode(self, data: bytes, redundancy: float = 0.05) -> List[bytes]:
        ...
        source = [padded[i * self.shard_size:(i + 1) * self.shard_size] for i in range(k)]
    ...
    def decode(self, shards: List[bytes], k: int = None) -> bytes:
        ...
        if orig_len is None or src_k is None:
            ...
            return b"".join(s[:k] if k else s for s in shards)[:k * self.shard_size if k else None]
        ...
        for idx, data in available:
            if idx < src_k:
                ...
                row = [1 if (seed[i % len(seed)] & (1 << (i % 8))) else 0 for i in range(src_k)]
                if sum(row) == 0:
                    row[r % src_k] = 1
