    ...
    async def _async_express(self, interest: Interest) -> Data:
        ...
        for attempt in range(_RETRIES + 1):
            try:
                ...
                return await asyncio.wait_for(_run(), timeout=(_TIMEOUT_MS / 1000) + 0.5)
            except (InterestNack, InterestTimeout, asyncio.TimeoutError, Exception) as e:
                backoff = 0.1 * (2 ** attempt)
                log.warning("NDN attempt %d failed: %s — retry in %.2fs", attempt, e, backoff)
                if attempt < _RETRIES:
                    await asyncio.sleep(backoff)
        return self._fallback(interest)

    def _fallback(self, interest: Interest) -> Data:
        content = self._fallback_content or (b"fallback_shard_" + interest.name.encode())
