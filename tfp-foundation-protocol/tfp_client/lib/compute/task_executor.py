...
class TaskSpec:
    ...
    expected_output_hash: str  # SHA3-256 hex of the correct result; used for verification
...
def generate_hash_preimage_task(
    task_id: str, difficulty: int, seed: bytes
) -> TaskSpec:
    """
    ...
    if nonce is None:
        raise TaskExecutionError("Could not generate solvable task within iteration limit")
...
def _execute_hash_preimage(spec: TaskSpec, deadline: float) -> Tuple[bytes, int]:
    ...
    while time.monotonic() < deadline:
        ...
        if candidate[:target_bytes_count] == b'\x00' * target_bytes_count:
            if target_bits == 0 or (candidate[target_bytes_count] & ~(0xFF >> target_bits)) == 0:
