...
from tfp_client.lib.bridges.nostr_bridge import NostrBridge, TFP_CONTENT_KIND
...
class TestPublishToIPFS:
    ...
    def test_pin_content_hash_matches_broadcaster(self):
        ...
        result = broadcaster.seed_content(content, metadata={"title": "IPFS Test"}, use_ldm=False)
...
class TestNostrAnnounceThenDiscover:
    def test_publish_event_received_by_subscriber(self):
        ...
        result = broadcaster.seed_content(content, metadata={"title": "Nostr Test"}, use_ldm=False)
    ...
    def test_enroll_earn_publish_search_get(self):
        ...
        with TestClient(app) as client:
            ...
            earn = client.post(
                ...
                headers={"X-Device-Sig": _make_sig(puf_entropy, f"{device_id}:{task_id}")},
            )
            ...
            pub = client.post(
                ...
                headers={"X-Device-Sig": _make_sig(puf_entropy, f"{device_id}:{title}")},
            )
