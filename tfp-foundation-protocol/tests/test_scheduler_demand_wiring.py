...
from tfp_broadcaster.src.gateway.scheduler import GatewayScheduler, BroadcastSlot
...
def _populated_aggregator(hashes_and_counts: dict, region: str = "test") -> MeshAggregator:
...
class TestScheduleFromAggregator:
    ...
    def test_respects_max_slots_per_epoch(self):
        agg = _populated_aggregator(
            {f"{i:064d}": (100 - i * 5) for i in range(10)}
        )
