...
from tfp_demo.server import TaskStore, _Metrics, app
        ...
        with TestClient(app) as client:
            ...
            before_count = int(
                [
                    ...
                    if l.startswith("tfp_tasks_created_total ")
                ][0].split()[1]
            )
            client.post(
                "/api/task",
...
class TestRealTaskExecution:
    def test_hash_preimage_executes_and_verifies(self):
        spec = generate_hash_preimage_task(
            "hp-test-1", difficulty=2, seed=b"test-seed-hp"
        )
    ...
    def test_matrix_verify_executes_and_verifies(self):
        spec = generate_matrix_verify_task(
            "mv-test-1", difficulty=2, seed=b"test-seed-mv"
        )
    ...
    def test_content_verify_executes_and_verifies(self):
        spec = generate_content_verify_task(
            "cv-test-1", difficulty=2, content=b"hello world content"
        )
        result = execute_task(spec, timeout_s=10.0)
        assert result.verified_locally
        assert result.output_hash == spec.expected_output_hash
    ...
    def test_tampered_expected_hash_fails_local_verify(self):
        spec = generate_hash_preimage_task(
            "hp-tamper", difficulty=1, seed=b"tamper-test"
        )
    ...
    def test_difficulty_1_faster_than_difficulty_5(self):
        ...
        t1 = time.monotonic()
        execute_task(spec1, timeout_s=30.0)
        t1 = time.monotonic() - t1
        t3 = time.monotonic()
        execute_task(spec3, timeout_s=60.0)
        t3 = time.monotonic() - t3
        assert (
            t3 >= t1
        )  # Higher difficulty should take longer (or equal in degenerate case)
...
class TestComputeCreditCycle:
    ...
    def test_three_device_consensus_mints_credits(self):
        ...
        with TestClient(app) as client:
            ...
            assert results[-1]["verified"] is True, (
                f"Expected consensus at 3rd submission, got: {results[-1]}"
            )
    ...
    def test_unenrolled_device_cannot_submit_result(self):
        with TestClient(app) as client:
            ...
            r = client.post(
                f"/api/task/{spec.task_id}/result",
                json={
                    "device_id": "ghost",
                    "output_hash": correct_hash,
                    "exec_time_s": 0.5,
...
class TestLegacyEarnPath:
    ...
    def test_no_credits_returns_402(self):
        with TestClient(app) as client:
            ...
            root_hash = all_content[0]["root_hash"]
            r = client.get(
                f"/api/get/{root_hash}", params={"device_id": "penniless-dev"}
            )
...
class TestContentFlow:
    def test_publish_and_retrieve_full_cycle(self):
        with TestClient(app) as client:
            ...
            client.post(
                "/api/earn",
                json={"device_id": "pub-dev", "task_id": task_id},
        ...
        with TestClient(app) as client:
            ...
            for spec in task_specs:
                ...
                assert consensus_reached, (
                    f"Consensus never reached for task {spec.task_id}"
                )
...
class TestDeviceLeaderboard:
    ...
    def test_leaderboard_sorted_by_credits(self):
        with TestClient(app) as client:
            ...
            sig = _hmac.new(
                e1, "lb-rich:lb-sort-task".encode(), hashlib.sha256
            ).hexdigest()
            client.post(
                "/api/earn",
                json={"device_id": "lb-rich", "task_id": "lb-sort-task"},
                headers={"X-Device-Sig": sig},
            )
            r = client.get("/api/devices")
            devices = r.json()["devices"]
            idx_rich = next(
                (i for i, d in enumerate(devices) if d["device_id"] == "lb-rich"), None
            )
            idx_poor = next(
                (i for i, d in enumerate(devices) if d["device_id"] == "lb-poor"), None
            )
...
class TestTaskExpiryReaper:
    ...
    def test_reap_expired_marks_failed(self):
        ...
        assert reaped >= 1
        row = conn.execute(
            "SELECT status FROM tasks WHERE task_id = 'reap-001'"
        ).fetchone()
        assert row[0] == "failed"
...
class TestMetricsDbSeed:
    ...
    def test_metrics_seeded_from_db(self):
        ...
        conn.execute(
            "CREATE TABLE IF NOT EXISTS content (root_hash TEXT PRIMARY KEY, title TEXT, tags TEXT, data BLOB)"
        )
        # Create supply_ledger
        conn.execute(
            "CREATE TABLE IF NOT EXISTS supply_ledger (id INTEGER PRIMARY KEY CHECK (id=1), total_minted INTEGER NOT NULL DEFAULT 0)"
        )
        conn.execute(
            "INSERT OR IGNORE INTO supply_ledger (id, total_minted) VALUES (1, 500)"
        )
        # Create tasks table
        conn.execute(
            "CREATE TABLE IF NOT EXISTS tasks (task_id TEXT PRIMARY KEY, task_type TEXT, difficulty INT, spec_json TEXT, status TEXT DEFAULT 'open', created_at REAL, deadline REAL, credit_reward INT DEFAULT 10)"
        )
        conn.execute(
            "INSERT INTO tasks VALUES ('t1','hash_preimage',1,'{}','completed',0,9999999,10)"
        )
        conn.execute(
            "INSERT INTO tasks VALUES ('t2','hash_preimage',1,'{}','failed',0,0,10)"
        )
        conn.execute(
            "CREATE TABLE IF NOT EXISTS task_results (result_id INTEGER PRIMARY KEY AUTOINCREMENT, task_id TEXT, device_id TEXT, output_hash TEXT, exec_time_s REAL, has_tee INT DEFAULT 0, submitted_at REAL, UNIQUE(task_id, device_id))"
        )
        conn.execute(
            "INSERT INTO task_results (task_id, device_id, output_hash, exec_time_s, has_tee, submitted_at) VALUES ('t1','seed-test-dev','aa'*32,0.1,0,0)"
        )
        conn.commit()
        m = _Metrics()
        m.seed_from_db(conn)
...
class TestHABPRestartSurvival:
    ...
    def test_habp_rebuilt_from_persisted_results(self):
        ...
        proof3 = generate_execution_proof(
            "restart-dev-2", task_id, bytes.fromhex(output_hash), 0.1
        )
...
class TestContentPagination:
    ...
    def test_limit_restricts_results(self):
        with TestClient(app) as client:
            ...
            for i in range(5):
                sig = _hmac.new(
                    entropy, f"pg-dev:Paging Test {i}".encode(), hashlib.sha256
                ).hexdigest()
                client.post(
                    "/api/publish",
                    json={
    ...
    def test_offset_pages_through_results(self):
        with TestClient(app) as client:
            ...
            for i in range(4):
                sig = _hmac.new(
                    entropy, f"pg2-dev:Offset Test {i}".encode(), hashlib.sha256
                ).hexdigest()
                client.post(
                    "/api/publish",
                    json={
            ...
            p1 = client.get(
                "/api/content", params={"tag": "offset-test", "limit": 2, "offset": 0}
            ).json()
            p2 = client.get(
                "/api/content", params={"tag": "offset-test", "limit": 2, "offset": 2}
            ).json()
    ...
    def test_tag_pagination_total_is_tag_count(self):
        with TestClient(app) as client:
            ...
            for i in range(3):
                sig = _hmac.new(
                    entropy, f"tag-pg-dev:Tag Page {i}".encode(), hashlib.sha256
                ).hexdigest()
                client.post(
                    "/api/publish",
                    json={
        ...
        with TestClient(app) as client:
            ...
            try:
                ...
                def fake_get(url, **kwargs):
                    ...
                    response = client.get(
                        path, **{k: v for k, v in kwargs.items() if k != "timeout"}
                    )
                    return response
        ...
        with TestClient(app) as client:
            ...
            try:
                ...
                args = parser.parse_args(
                    ["--api", "http://127.0.0.1:8000", "leaderboard"]
                )
                buf = io.StringIO()
                with redirect_stdout(buf):
...
class TestMultiTagSearch:
    ...
    def test_multi_tag_union_returns_superset(self):
        with TestClient(app) as client:
            ...
            sig_a = _hmac.new(
                entropy, b"multi-tag-dev:Alpha Article", hashlib.sha256
            ).hexdigest()
            client.post(
                "/api/publish",
                json={
                    ...
                    "text": "content",
                    "tags": ["alpha-mt"],
                    "device_id": "multi-tag-dev",
                },
                headers={"X-Device-Sig": sig_a},
            )

            sig_b = _hmac.new(
                entropy, b"multi-tag-dev:Beta Article", hashlib.sha256
            ).hexdigest()
            client.post(
                "/api/publish",
                json={
            ...
            r_both = client.get(
                "/api/content", params={"tags": "alpha-mt,beta-mt"}
            ).json()
    ...
    def test_tags_param_overrides_tag(self):
        with TestClient(app) as client:
            ...
            sig = _hmac.new(
                entropy, b"override-dev:Override Test", hashlib.sha256
            ).hexdigest()
            client.post(
                "/api/publish",
                json={
            ...
            r = client.get(
                "/api/content",
                params={"tag": "nonexistent", "tags": "override-unique-zz9"},
            )
            assert r.status_code == 200
    ...
    def test_rate_limit_fires_on_excessive_submissions(self):
        ...
        try:
            with TestClient(app) as client:
                ...
                _srv._result_rate_limiter = _srv._RateLimiter(
                ...
                assert statuses[2] == 429, (
                    f"Expected 429 for 3rd call, statuses={statuses}"
                )
        finally:
...
class TestCLISearchTableOutput:
        ...
        with TestClient(app) as client:
            ...
            try:
                args = build_parser().parse_args(
                    ["--api", "http://127.0.0.1:8000", "search", "--limit", "5"]
                )
                buf = io.StringIO()
                with redirect_stdout(buf):
    ...
    def test_search_multi_tag_cli(self):
        ...
        with TestClient(app) as client:
            ...
            sig = _hmac.new(
                entropy, b"cli-search-multi:CLI Multi Tag", hashlib.sha256
            ).hexdigest()
            client.post(
                "/api/publish",
                json={
