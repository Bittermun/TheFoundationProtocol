...
from tfp_demo.server import app, EarnLog, CreditStore, _RateLimiter
...
def test_credits_persist_across_restarts():
    ...
    try:
        ...
        with TestClient(app) as client:
            ...
            pub = client.post(
                "/api/publish",
                json={"title": title, "text": "data", "tags": [], "device_id": device_id},
...
def test_credit_store_class_save_and_load():
    ...
    from tfp_demo.server import DemoNDNAdapter, ContentStore, StoredContent
...
def test_real_adapters_importable():
    ...
    for cls in (RealNDNAdapter, RealRaptorQAdapter, RealZKPAdapter, RealMulticastAdapter):
        assert callable(cls)
