...
from tfp_client.lib.ndn.adapter import NDNAdapter, Data, Interest
...
def test_sync_via_ndn_calls_raptorq():
    ...
    mock_ndn.express_interest.return_value = Data(name="/tfp/lexicon/prefix", content=b"shard_data")
    mock_rq = MagicMock(spec=RaptorQAdapter)
    mock_rq.decode.return_value = b"decoded_data"
    result = tree.sync_via_ndn("/tfp/lexicon/prefix", ndn_adapter=mock_ndn, raptorq_adapter=mock_rq)
