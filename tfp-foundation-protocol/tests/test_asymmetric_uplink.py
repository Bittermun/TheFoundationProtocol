...
def test_choose_uplink_returns_channel_id():
    ...
    channels = [
        ...
        ChannelMetrics(
            channel_id=CHANNEL_WIFI_MESH, latency=20.0, energy=3.0, drop_rate=0.05
        ),
        ChannelMetrics(
            channel_id=CHANNEL_LEO, latency=50.0, energy=10.0, drop_rate=0.02
        ),
    ]
...
def test_lowest_cost_wins():
    ...
    channels = [
        ChannelMetrics(
            channel_id=CHANNEL_5G, latency=100.0, energy=100.0, drop_rate=0.1
        ),
        ChannelMetrics(
            channel_id=CHANNEL_WIFI_MESH, latency=1.0, energy=1.0, drop_rate=0.01
        ),
        ChannelMetrics(
            channel_id=CHANNEL_LEO, latency=200.0, energy=200.0, drop_rate=0.2
        ),
    ]
    result = router.choose_uplink_channel(channels)
...
def test_exponential_backoff_on_high_drop_rate():
    router = AsymmetricUplinkRouter()
    high_drop = ChannelMetrics(
        channel_id=CHANNEL_5G, latency=10.0, energy=5.0, drop_rate=0.8
    )
    low_drop = ChannelMetrics(
        channel_id=CHANNEL_5G, latency=10.0, energy=5.0, drop_rate=0.1
    )
...
def test_all_channels_equal_returns_first():
    ...
    channels = [
        ...
        ChannelMetrics(
            channel_id=CHANNEL_WIFI_MESH, latency=10.0, energy=5.0, drop_rate=0.1
        ),
