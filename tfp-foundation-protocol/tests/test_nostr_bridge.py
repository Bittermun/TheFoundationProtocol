...
pytestmark = pytest.mark.skipif(
    not NOSTR_AVAILABLE,
    reason="NostrBridge not yet implemented"
)

SAMPLE_HASH = "b" * 64
SAMPLE_METADATA = {"title": "Test Content", "tags": ["education", "science"], "domain": "science"}
...
class TestNostrEventConstruction:
    ...
    def test_event_kind_preserved(self):
        privkey = b"\x05" * 32
        event = NostrEvent.create(privkey=privkey, kind=TFP_CONTENT_KIND, content="x", tags=[])
    ...
    def test_event_id_deterministic(self):
        ...
        privkey = b"\x08" * 32
        ts = 1_700_000_000
        event_a = NostrEvent.create(privkey=privkey, kind=1, content="abc", tags=[], created_at=ts)
        event_b = NostrEvent.create(privkey=privkey, kind=1, content="abc", tags=[], created_at=ts)
    ...
    def test_different_content_different_id(self):
        ...
        ts = 1_700_000_000
        event_a = NostrEvent.create(privkey=privkey, kind=1, content="abc", tags=[], created_at=ts)
        event_b = NostrEvent.create(privkey=privkey, kind=1, content="xyz", tags=[], created_at=ts)
        assert event_a.id != event_b.id
...
class TestNostrPublishAnnouncement:
    ...
    def test_publish_with_mocked_relay(self):
        bridge = NostrBridge(offline=False, privkey=b"\x02" * 32, relay_url="wss://test")
    ...
    def test_publish_handles_relay_error(self):
        ...
        with patch.object(bridge, "_send_to_relay", side_effect=ConnectionError("refused")):
            result = bridge.publish_content_announcement(SAMPLE_HASH, SAMPLE_METADATA)
