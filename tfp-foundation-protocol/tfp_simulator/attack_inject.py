...
from tfp_client.lib.routing.asymmetric_uplink.router import AsymmetricUplinkRouter, ChannelMetrics
...
class ScenarioResult:
    ...
    def print(self, threshold: float = 0.0, pass_label: str = "PASS"):
        ...
        if threshold > 0:
            ...
            print(f"  {pass_label} [{bar}] : {verdict} (threshold={threshold*100:.0f}%)")
...
def run_popularity_persistence(rng: random.Random, n_nodes: int = 100) -> ScenarioResult:
    """
    ...
    for i in range(n_nodes):
        channels = [
            ...
            ChannelMetrics(channel_id=1, latency=30.0,  energy=50,  drop_rate=DROP_RATE * 0.5),
            ChannelMetrics(channel_id=2, latency=250.0, energy=80,  drop_rate=0.08),  # LEO
        ]
