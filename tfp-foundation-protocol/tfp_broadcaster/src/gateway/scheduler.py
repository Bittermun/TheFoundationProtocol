...
class GatewayScheduler:
    ...
    def __init__(self, base_credit_rate: int = 100, 
                 max_slots_per_epoch: int = 10):
        """
        ...
        self._slots: Dict[int, List[BroadcastSlot]] = defaultdict(list)  # epoch -> slots
        self._pending_bids: Dict[str, Dict[str, Any]] = {}  # hash -> bid info
        self._next_slot_id: int = 0
        self._current_load: Dict[int, float] = defaultdict(float)  # epoch -> load factor
    ...
    def calculate_bid(self, content_hash: str, demand_score: float,
                      content_size: int = 0, current_load: float = 0.0) -> int:
        """
    ...
    def schedule_broadcast_slot(self, content_hash: str, bid: int, 
                                 epoch: int) -> Optional[BroadcastSlot]:
        """
    ...
    def schedule_from_demand(self, epoch: int, 
                             max_slots: Optional[int] = None) -> List[BroadcastSlot]:
        """
        ...
        for hash_hex, bid_info in sorted_bids[:slots_available]:
            # Calculate bid
            credit_bid = self.calculate_bid(
                hash_hex,
                bid_info["demand_score"]
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        broadcasted = sum(
            ...
            for slot in slots if slot.status == "broadcasted"
        )
        ...
        return {
            ...
            "avg_load": sum(self._current_load.values()) / len(self._current_load) if self._current_load else 0.0
        }
    ...
    def export_schedule(self, epoch: int) -> bytes:
        ...
        return json.dumps({
            "epoch": epoch,
            "slots": schedule,
            "load": self.get_epoch_load(epoch)
        }).encode('utf-8')
