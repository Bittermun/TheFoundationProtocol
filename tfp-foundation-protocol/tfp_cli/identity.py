...
def generate_mnemonic(entropy_bytes: Optional[bytes] = None) -> str:
    if entropy_bytes is None:
        entropy = secrets.token_bytes(16)
    else:
        entropy = entropy_bytes
    if len(entropy) != 16:
        raise ValueError('BIP-39 12-word mnemonic requires 16 bytes entropy')
    cs = hashlib.sha256(entropy).digest()[0] >> 4
