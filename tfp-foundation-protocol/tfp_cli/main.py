...
def _load_or_create_identity(device_id: str) -> dict:
    ...
    if os.path.exists(path):
        ...
        if device_id in identities:
            ...
            return {"device_id": device_id, "puf_entropy": bytes.fromhex(entry["puf_entropy_hex"])}
...
def cmd_publish(args) -> int:
    ...
    payload = {"title": args.title, "text": args.text, "tags": tags, "device_id": args.device_id}
    sig = _make_sig(puf_entropy, f"{args.device_id}:{args.title}")
    response = httpx.post(
        f"{args.api}/api/publish", json=payload,
        headers={"X-Device-Sig": sig}, timeout=10,
    )
...
def cmd_earn(args) -> int:
    ...
    response = httpx.post(
        f"{args.api}/api/earn", json=payload,
        headers={"X-Device-Sig": sig}, timeout=10,
    )
    if response.status_code >= 400:
        return _handle_error(response)
    ...
    try:
        while True:
            ...
            if resp.status_code != 200:
                print(f"[join] WARN: /api/tasks returned {resp.status_code}, retrying in {args.interval}s")
            ...
            print(f"[join] Executing task {task_id} (type={task_info['task_type']}, diff={task_info['difficulty']}) …")

            # Fetch full spec
            detail_resp = httpx.get(f"{args.api}/api/task/{task_id}", timeout=10)
            if detail_resp.status_code != 200:
                print(f"[join] WARN: could not fetch task spec, skipping")
                time.sleep(2)
                continue
            ...
            try:
                ...
                print(f"[join]   ✓ executed in {elapsed:.2f}s — output_hash={result.output_hash[:16]}…")
            ...
            if submit_resp.status_code == 200:
                ...
                tasks_done += 1
                if v.get("verified"):
                    print(f"[join]   💰 CONSENSUS REACHED — earned {credits_earned} credits (total: {total_credits})")
                else:
                    needed = v.get("consensus_needed", "?")
                    print(f"[join]   ⏳ pending consensus ({needed} more proofs needed)")
            else:
                print(f"[join]   WARN: submit returned {submit_resp.status_code}: {submit_resp.text[:80]}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    publish.add_argument("--device-id", default="cli-user", dest="device_id", help="Device id")
    publish.set_defaults(func=cmd_publish)

    get = sub.add_parser("get", help="Fetch content by root hash")
    get.add_argument("root_hash", help="Content root hash")
    get.add_argument("--device-id", default="cli-user", dest="device_id", help="Device id for credit accounting")
    get.set_defaults(func=cmd_get)

    earn = sub.add_parser("earn", help="Earn credits by submitting compute task")
    earn.add_argument("--task-id", required=True, dest="task_id", help="Task recipe identifier")
    ...
    join.add_argument("--timeout", type=float, default=30.0,
