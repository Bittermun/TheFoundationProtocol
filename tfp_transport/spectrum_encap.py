...
class SpectrumMask:
    ...
    def is_compliant(self, measured_power_dbm: float, frequency_offset_mhz: float) -> Tuple[bool, str]:
        """
        ...
        if measured_power_dbm > self.max_power_dbm:
            return False, f"Power {measured_power_dbm:.1f} dBm exceeds limit {self.max_power_dbm:.1f} dBm"
        
        # Simplified mask check (production would use full mask curve)
        if abs(frequency_offset_mhz) > self.bandwidth_mhz / 2:
            # Out-of-band check
            if measured_power_dbm > (self.max_power_dbm + self.out_of_band_emission_dbc):
                return False, f"Out-of-band emission violates mask"
...
class SpectrumEncapsulator:
    ...
    def validate_modulation_mask(
        ...
        if not mask:
            return False, f"No spectrum mask defined for {self.region}/{self.active_standard.value}"
        ...
        is_compliant, reason = mask.is_compliant(
            measured_power_dbm,
            frequency_mhz - mask.center_frequency_mhz
        )
    ...
    def generate_compliance_report(self) -> dict:
        ...
        return {
            'timestamp': time.time(),
            'region': self.region,
            'active_standard': self.active_standard.value if self.active_standard else 'NONE',
            'total_events': total_count,
            'compliant_events': compliant_count,
            'compliance_rate': compliant_count / total_count if total_count > 0 else 1.0,
            'pii_logged': False,
            'recent_violations': [
                {
