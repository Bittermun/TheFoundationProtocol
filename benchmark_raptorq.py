...
def benchmark_encoding():
    ...
    for r in results:
        if r["label"] == "Medium file":
            ...
            log(
                f"  Full retrieval: {r['encoded_bytes'] / 1024 / 1024:.2f} MB (with 10% redundancy)"
            )
            log(
                f"  50% shards: {(r['encoded_bytes'] * 0.5) / 1024 / 1024:.2f} MB (50% bandwidth)"
            )
            log(
                f"  75% shards: {(r['encoded_bytes'] * 0.75) / 1024 / 1024:.2f} MB (75% bandwidth)"
            )
            log("  Savings vs full transfer: up to 25% with partial retrieval")
            log("")
