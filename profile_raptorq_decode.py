...
def profile_decoding(
    data: bytes, redundancy: float = 0.05, loss_rate: float = 0.0
) -> dict:
...
def main():
    ...
    for size_bytes, size_label in test_sizes:
        ...
        for redundancy in redundancies:
            for loss_rate in loss_rates:
                print(
                    f"  Redundancy {redundancy * 100:.0f}% Loss {loss_rate * 100:.0f}%...",
                    end=" ",
                    flush=True,
                )
                ...
                try:
                    ...
                    if result["success"]:
                        print(
                            f"{result['elapsed_ms']:.1f}ms ({result['throughput_mbps']:.2f} MB/s)"
                        )
                    else:
    ...
    print("=" * 70)
    print(
        f"{'Size':<10} {'Redundancy':<12} {'Time (ms)':<12} {'Throughput (MB/s)':<20}"
    )
    print("-" * 70)
    ...
    for r in results:
        ...
        if r["success"]:
            print(
                f"{size_label:<10} {r['redundancy'] * 100:>10.0f}% {r['elapsed_ms']:>10.1f} {r['throughput_mbps']:>18.2f}"
            )
        else:
    ...
    large_results = [
        r for r in results if r["data_size_bytes"] >= 10 * 1024 * 1024 and r["success"]
    ]
    if large_results:
        avg_throughput = sum(r["throughput_mbps"] for r in large_results) / len(
            large_results
        )
        print(
