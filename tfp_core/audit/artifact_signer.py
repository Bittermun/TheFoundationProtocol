...
try:
    ...
    from sigstore.verify import Verifier, VerificationPolicy
...
class ArtifactSigner:
    ...
    def __init__(self, mode: str = "keyless", issuer: str = "https://oauth2.sigstore.dev/auth"):
        """
        ...
        if SIGSTORE_AVAILABLE:
            ...
            logger.warning("Sigstore library not available - signing will use mock mode")
            self._ctx = None
    ...
    def sign(self, data: bytes) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.warning("Actual Sigstore signing requires OIDC token - returning mock bundle")
            return {
    ...
    def verify(self, data: bytes, bundle: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning("Actual verification requires real bundle - returning True for mock")
