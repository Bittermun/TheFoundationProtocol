...
class AuditValidator:
    ...
    def run_code_coverage(self) -> Dict[str, Any]:
        ...
        """
        try:
            # Validate repo_path to prevent command injection via malicious cwd
            ...
            # Ensure repo_path is within expected workspace boundaries
            ...
            try:
                ...
                return {
                    "status": "error",
                    "error": f"Repository path must be within {workspace_root}, got {resolved_path}",
                    "total_coverage": 0.0,
                    "meets_target": False,
                }
    ...
    def run_security_scan(self) -> Dict[str, Any]:
        ...
        try:
            # Validate repo_path to prevent command injection
            if not self.repo_path.exists():
                return {
                    "status": "error",
                    "error": f"Repository path does not exist: {self.repo_path}",
            ...
            # Ensure repo_path is within workspace boundaries
            resolved_path = self.repo_path.resolve()
            workspace_root = Path("/workspace").resolve()
            try:
                resolved_path.relative_to(workspace_root)
            ...
            # Run bandit with validated path - only scan tfp_core subdirectory
            bandit_target = resolved_path / "tfp_core"
            if not bandit_target.exists():
                bandit_target = resolved_path

            bandit_result = subprocess.run(
                ["bandit", "-r", str(bandit_target), "-f", "json"],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
            ...
            issues_found.append({"tool": "bandit", "error": "Security scan timed out"})
        except Exception as e:
            issues_found.append({"tool": "bandit", "error": str(e)})

        try:
            # Run safety with validated path
            ...
            if safety_result.stdout:
                ...
                for vuln in vulns:
                    vulnerabilities["high"] += (
                        1  # Safety typically reports high-severity
                    )
                    vulnerabilities["total"] += 1
            ...
            issues_found.append(
                {"tool": "safety", "error": "Dependency check timed out"}
            )
        except Exception as e:
    ...
    def analyze_architecture(self) -> Dict[str, Any]:
        ...
        for module_dir in [
            ...
            if module_path.exists():
                ...
                modules[module_dir] = {"files": len(py_files), "loc": loc}
                total_loc += loc
    ...
    def save_audit_report(self, filepath: str = "AUDIT_REPORT.json") -> None:
        ...
        print(
            f"🛡️  SECURITY: {'✓ PASS' if report['security']['zero_critical_high'] else '✗ ISSUES FOUND'}"
        )
        print(
            f"🏗️  ARCHITECTURE: {report['architecture']['modular_design_score']} modularity"
        )
