    ...
    def run(self, repo_path: str) -> Dict[str, Any]:
        ...
        for check_name, score in all_checks.items():
            check_results.append({
                "name": check_name,
                "score": score,
                "reason": f"Assessed based on TFP v3.2 implementation"
            })
            total_score += score
        ...
        if not passed:
            results["warning"] = f"Security score {avg_score:.2f} is below threshold {self.threshold}"
            logger.warning(results["warning"])
        else:
            logger.info(f"Security score {avg_score:.2f} meets threshold {self.threshold}")
