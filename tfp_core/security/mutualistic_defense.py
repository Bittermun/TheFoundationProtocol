...
class AuditorProfile:
    ...
    domain_expertise: Dict[str, float] = field(default_factory=dict)  # category -> score
...
class LocalTrustCache:
    ...
    def _evict_lowest_trust(self):
        ...
        candidates = [
            (aid, prof) for aid, prof in self.auditors.items()
    ...
    def get_trusted_auditors(self, category: str = None, min_level: TrustLevel = TrustLevel.TRUSTED) -> List[str]:
        """Get list of trusted auditors, optionally filtered by domain."""
        result = []
        for aid, profile in self.auditors.items():
            if profile.trust_level.value >= min_level.value and not profile.is_on_cooldown():
...
class GossipVerifier:
    ...
    def aggregate_signals(self, auditor_id: str) -> Tuple[float, int]:
        ...
        recent = [
            ...
            if s['auditor'] == auditor_id and
            (time.time() - s['timestamp']) / 3600.0 < self.signal_expiry_hours
        ]
        if not recent:
            return 0.5, 0
...
class MutualisticAuditor:
    ...
    def audit_content(
        ...
        # Randomized sampling for low-volume content
        should_audit = (
            request_count >= 100 or  # High-demand: always audit
            random.random() < self.sample_rate_low_volume  # Low-demand: random sample
        )
...
if __name__ == '__main__':
    ...
    auditor.report_audit_outcome('volunteer_auditor', was_correct=True, category='video')
