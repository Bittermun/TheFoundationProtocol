...
class CryptoExportGate:
    ...
    def __init__(self):
        ...
        self.restricted_countries = {
            ...
            "LV",
            "EE",
            "FI",  # Some EU countries have additional restrictions
        }
    ...
    def detect_jurisdiction(
        ...
        self.current_heuristic = JurisdictionHeuristic(
            category=category, confidence=confidence, detection_method=method
        )
    ...
    def negotiate_suite(
        self, requested_algorithms: Dict[str, List[str]] = None
    ) -> NegotiatedSuite:
        """
    ...
    def get_approved_algorithm(
        self, algo_type: str, preferred: str = None
    ) -> Optional[str]:
        """
    ...
    def is_compliant(self) -> Tuple[bool, str]:
        ...
        return (
            True,
            f"Compliant for {self.current_heuristic.category.value} jurisdiction",
        )
    ...
    def generate_compliance_report(self) -> dict:
        ...
        return {
            "timestamp": time.time(),
            "jurisdiction": self.current_heuristic.category.value
            if self.current_heuristic
            else "UNKNOWN",
            "detection_method": self.current_heuristic.detection_method
            if self.current_heuristic
            else "NONE",
            "negotiated_suite": self.negotiated_suite.suite_name
            if self.negotiated_suite
            else "NONE",
            "approved_algorithms": self.negotiated_suite.algorithms
            if self.negotiated_suite
            else {},
...
if __name__ == "__main__":
    ...
    print(
        f"Jurisdiction: {heuristic.category.value} ({heuristic.confidence:.0%} confidence)"
    )
