...
class TaskMeshGates:
    ...
    def can_accept_task(
        ...
        with self._lock:
            ...
            if device_level < required_level:
                return False, f"Insufficient capability: {device_cap.value} < {task.required_capability.value}"
    ...
    def submit_result(self, result: TaskResult) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            # Check for consensus
            if not record.is_consensus and len(record.results) >= self.consensus_threshold:
                ...
                for output_hash, count in hash_counts.items():
                    if count >= self.consensus_threshold:
                        ...
                        matching_devices = [
                            r.device_id for r in record.results
    ...
    def get_stake_balance(self, device_id: str) -> float:
        ...
        with self._lock:
            ...
            current_balance = stake.staked_amount * decay_factor + stake.total_earned - stake.total_slashed
...
if __name__ == "__main__":
    ...
    for device_id in ["device_B", "device_C", "device_E"]:
        ...
        print(f"  {device_id}: verified={response['task_verified']}, yield={response.get('credit_yield', 0):.3f}")
