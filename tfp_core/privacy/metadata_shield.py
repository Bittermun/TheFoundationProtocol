...
class MetadataShield:
    ...
    def generate_dummy_interests(self, count: Optional[int] = None) -> List[str]:
        ...
        for _ in range(count):
            ...
            random_bytes = hashlib.sha3_256(
                f"dummy_{time.time()}_{random.random()}".encode()
            ).digest()[:16]
    ...
    def get_privacy_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "active_backoffs": len([
                    t for t in self._backoff_state.values()
                    if t > time.time()
                ])
            }
    ...
    def _prune_old_records(self, current_time: float) -> None:
        ...
        to_remove = [
            h for h, record in self._recent_interests.items()
        ...
        expired_backoffs = [
            h for h, t in self._backoff_state.items()
            if t < current_time
        ]
