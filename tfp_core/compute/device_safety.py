...
class DeviceSafetyGuard:
    ...
    def check_safety(
        self, metrics: DeviceMetrics, device_id: str = "default"
    ) -> SafetyCheckResult:
        """
        ...
        if metrics.battery_level < self._min_battery:
            if not metrics.is_charging:
                warnings.append(
                    f"Battery low ({metrics.battery_level}%) and not charging"
                )
        ...
        if metrics.uptime_hours >= self._max_uptime_before_rest:
            warnings.append(
                f"Extended uptime ({metrics.uptime_hours:.1f}h) - rest recommended"
            )
            status = SafetyStatus.WARNING
            can_accept = False
