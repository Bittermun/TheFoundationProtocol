...
class CryptoSuite:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "CryptoSuite":
        return cls(
            ...
            kem_algo=CryptoAlgorithm(data["kem_algo"])
            if data.get("kem_algo")
            else None,
...
class CryptoAgilityRegistry:
    ...
    def register_suite(self, suite: CryptoSuite) -> bool:
        ...
        if suite.suite_id in self._suites:
            ...
            if suite.version <= existing.version:
                logger.warning(
                    f"Suite {suite.suite_id} v{suite.version} not newer than existing v{existing.version}"
                )
                return False
    ...
    def negotiate_suite(
        ...
        if cache_key in self._negotiation_cache:
            ...
            if (
                time.time() - cached.selected_suite.created_at < 300
                if cached.selected_suite
                else False
            ):
                return cached
    ...
    def _algo_compatible(
        self, suite: CryptoSuite, device_algos: List[CryptoAlgorithm]
    ) -> bool:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "deprecated_suites": sum(
                1 for s in self._suites.values() if s.is_deprecated
            ),
        }
...
def verify_signature(
    data: bytes, signature: bytes, suite_id: str, public_key: bytes
) -> bool:
    """
