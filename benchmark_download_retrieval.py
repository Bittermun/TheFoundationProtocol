...
class DownloadBenchmarkClient:
    ...
    def publish_content(self, data: bytes, title: str = "Benchmark Content", tags: List[str] = None) -> str:
        ...
        try:
            ...
            files = {
                "content": ("content.bin", data, "application/octet-stream")
            }
            data_dict = {
                "device_id": self.device_id,
                "title": title,
    ...
    def download_content(
        ...
        try:
            ...
            response = self._client.get(
                f"{self.base_url}/api/get/{root_hash}",
                params=params,
                headers=headers
            )
        ...
        return DownloadMetrics(
            ...
            success=success,
            error=error,
            credits_spent=1 if success else 0,  # Only spend credits on successful downloads
    ...
    async def download_concurrent(
        ...
        async def single_download():
            return await asyncio.to_thread(self.download_content, root_hash, stream=stream)
        ...
        for r in results:
            if isinstance(r, Exception):
                final_results.append(DownloadMetrics(
                    download_type="streaming" if stream else "non-streaming",
                    file_size_bytes=0,
                    range_request=False,
    ...
    def download_with_range(
        ...
        return self.download_content(root_hash, stream=stream, range_header=range_header)
...
def run_benchmark(
    ...
    if credits == 0:
        ...
        return results
    if credits < 50:
        print(f"WARNING: Only earned {credits} credits, may be insufficient for all tests")
    ...
    for file_size in file_sizes:
        ...
        root_hash = client.publish_content(data, title=f"Benchmark {file_size/1024:.0f}KB")
        if not root_hash:
            ...
            for stream_mode in [True, False]:
                ...
                if warmup_iterations > 0:
                    ...
                    if concurrency == 1:
                        ...
                        async def warmup_downloads():
                            tasks = [client.download_concurrent(root_hash, concurrency, stream_mode) for _ in range(warmup_iterations)]
                ...
                if concurrency == 1:
                    for i in range(iterations):
                        ...
                        print(f"  Iteration {i+1}/{iterations}: Success {success_count}/{len(all_metrics)}")
                else:
                    # Run all benchmark iterations in single async call
                    async def benchmark_downloads():
                        tasks = [client.download_concurrent(root_hash, concurrency, stream_mode) for _ in range(iterations)]
                        results = await asyncio.gather(*tasks)
                        return [item for sublist in results for item in sublist]
                    ...
                    success_count = sum(1 for m in all_metrics if m.success)
                    print(f"  Completed {iterations} iterations: Success {success_count}/{len(all_metrics)}")
                ...
                print(f"  p50/p95/p99: {result.p50_time_ms:.1f}ms / {result.p95_time_ms:.1f}ms / {result.p99_time_ms:.1f}ms")
        ...
        if file_size >= 1048576:  # 1MB
            ...
            for range_name, range_start, range_end in range_scenarios:
                ...
                if warmup_iterations > 0:
                    print(f"Warmup ({warmup_iterations} iterations)...")
                    for _ in range(warmup_iterations):
                        metrics = client.download_with_range(root_hash, range_start, range_end, stream=True)
                        all_metrics.append(metrics)
...
def main():
    ...
    parser.add_argument("--node", default="http://localhost:9001", help="Testbed node URL")
    parser.add_argument("--iterations", type=int, default=3, help="Number of iterations per scenario")
    parser.add_argument("--warmup", type=int, default=5, help="Warmup iterations")
    parser.add_argument("--output", help="Output JSON file")
    parser.add_argument("--file-sizes", nargs="+", type=int, default=[102400, 524288, 1048576, 10485760],
    ...
    print(f"Initial resources: CPU {initial_resources.cpu_percent:.1f}%, Memory {initial_resources.memory_mb:.1f}MB")
    ...
    for result in results:
        ...
        print(f"  p50/p95/p99 latency: {result.p50_time_ms:.1f}ms / {result.p95_time_ms:.1f}ms / {result.p99_time_ms:.1f}ms")
        print(f"  Success rate: {result.success_count}/{result.iterations}")
