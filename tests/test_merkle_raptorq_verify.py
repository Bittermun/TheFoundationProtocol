...
class TestMerkleTree(unittest.TestCase):
    ...
    def test_build_merkle_tree(self):
        ...
        leaf_hashes = [
            h.hex()
            for h in [__import__("hashlib").sha3_256(d).digest() for d in leaf_data]
        ]
...
class TestMerkleizedRaptorQ(unittest.TestCase):
    ...
    def test_register_content(self):
        ...
        self.assertEqual(
            self.tree.root_hash[:8], self.tree.root_hash[:8]
        )  # Has root hash
    ...
    def test_verify_shard_no_tree(self):
        ...
        mac = __import__("hashlib").sha3_256(b"unknown_content:0:" + b"data").digest()
    ...
    def test_get_verified_shards(self):
        ...
        for i, data in enumerate(shard_data):
            ...
            mac = (
                __import__("hashlib")
                .sha3_256(f"{content_hash}:{i}:".encode() + data)
                .digest()
            )
    ...
    def test_integrity_stats(self):
        ...
        self.assertEqual(
            stats["pending_admissions"], 3
        )  # All pending (1 convergence each, need 2)
...
class TestTransportIntegration(unittest.TestCase):
    ...
    def test_poisoned_shard_rejection(self):
        ...
        for shard_id, data, should_pass in test_cases:
            ...
            mac = (
                __import__("hashlib")
                .sha3_256(f"{content_hash}:{shard_id}:".encode() + data)
                .digest()
            )
