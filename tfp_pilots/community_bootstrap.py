...
class GhostNode:
    ...
    def __init__(self, node_id: str, region: str, content_hashes: List[str]):
        ...
        self.online = True
        self.latency_ms = random.randint(20, 200)  # Realistic latency variation
    ...
    def get_latency(self) -> int:
        ...
        return self.latency_ms + random.randint(-10, 10)
...
class CommunityBootstrap:
    ...
    def create_ghost_network(self, region: str, content_library: List[Dict], 
                              node_count: int = 10) -> None:
        """
        ...
        for i in range(node_count):
            ...
            ghost_node = GhostNode(
                node_id=node_id,
                region=region,
                content_hashes=node_content
            )
            self.ghost_nodes.append(ghost_node)
        
        print(f"✓ Created {node_count} ghost nodes")
        print(f"  Total content items: {len(content_library)}")
        print(f"  Average content per node: {sum(len(n.content_hashes) for n in self.ghost_nodes) / node_count:.0f}")
    
    def simulate_content_request(self, content_hash: str, user_region: str) -> Dict[str, Any]:
        """
    ...
    def load_community_content(self, content_file: str) -> List[Dict]:
        ...
        return [
            ...
            {"hash": "local_music_track_01", "category": "entertainment", "size_kb": 3000},
    ...
    def generate_pilot_report(self) -> Dict[str, Any]:
        ...
        for content_hash in total_content:
            nodes_with_content = sum(1 for n in self.ghost_nodes if n.has_content(content_hash))
        ...
        return {
            ...
            "average_latency_ms": sum(n.latency_ms for n in self.ghost_nodes) / len(self.ghost_nodes),
    ...
    def _generate_recommendations(self, availability_stats: Dict) -> List[str]:
        ...
        if highly_available < 5:
            recommendations.append("Increase ghost node count to improve content redundancy")
        
        if rare > len(availability_stats) * 0.3:
            recommendations.append("Focus content library on high-demand items; reduce long-tail content")
        
        if len(self.ghost_nodes) >= 10:
            recommendations.append("Ghost network sufficient for pilot; proceed to real-user testing")
...
def main():
    ...
    content_library = bootstrap.load_community_content("/workspace/tfp_pilots/nairobi_content.json")
    ...
    test_hashes = ["emergency_weather_001", "health_malaria_guide", "local_music_track_01"]
    
    for content_hash in test_hashes:
        result = bootstrap.simulate_content_request(content_hash, "Nairobi, Kenya")
        if result["success"]:
            print(f"  ✓ {content_hash}: served by {result['serving_node']} in {result['latency_ms']:.0f}ms")
        else:
    ...
    print(f"  • Highly available: {report['availability_distribution'].get('highly_available', 0)} items")
