...
def generate_mnemonic(entropy_bytes: Optional[bytes] = None, strength: int = 256) -> str:
    """Generate standardized BIP-39 mnemonic supporting 128-bit (12 words) to 256-bit (24 words) entropy."""
    try:
        from tfp_core.crypto.bip39 import generate_mnemonic as _gen_bip39
        return _gen_bip39(strength=strength, entropy_bytes=entropy_bytes)
    except Exception:
        # Fallback implementation
        if entropy_bytes is None:
            entropy = secrets.token_bytes(strength // 8 if strength in (128, 160, 192, 224, 256) else 32)
        else:
            entropy = entropy_bytes
        if len(entropy) not in (16, 20, 24, 28, 32):
            raise ValueError(f"BIP-39 mnemonic requires 16, 20, 24, 28, or 32 bytes entropy (got {len(entropy)})")
        ...
        combined = (entropy_int << cs_bits) | cs_int
...
def validate_mnemonic(mnemonic: str) -> bool:
    """Validate BIP-39 mnemonic wordlist membership and checksum."""
    try:
        from tfp_core.crypto.bip39 import validate_mnemonic as _val_bip39
        return _val_bip39(mnemonic)
    except Exception:
        ...
        if len(words) not in (12, 15, 18, 21, 24):
            return False
        word_map = {w: i for i, w in enumerate(BIP39_WORDLIST)}
        combined = 0
        for w in words:
            if w not in word_map:
                return False
            combined = (combined << 11) | word_map[w]
        total_bits = len(words) * 11
        cs_bits = total_bits // 33
        ent_bits = total_bits - cs_bits
        extracted_cs = combined & ((1 << cs_bits) - 1)
...
def mnemonic_to_seed(mnemonic: str, passphrase: str = "") -> bytes:
    """Derive 64-byte binary seed from mnemonic using PBKDF2-HMAC-SHA512 with 2048 iterations."""
