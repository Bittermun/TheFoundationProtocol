...
class VirtualDevice:
    ...
    def tick(self, delta_seconds: float) -> None:
        ...
        with self._lock:
            ...
            if self.state.state == DeviceState.COMPUTING:
                heat_gen = (self.state.current_load_pct / 100.0) * 2.5  # Degrees per sec
                cooling = 0.8  # Passive cooling
                self.state.current_temp_celsius += (heat_gen - cooling) * delta_seconds
                self.state.battery_level_pct -= (self.state.current_load_pct / 100.0) * 0.05 * delta_seconds
            else:
                # Cooling down when idle
                self.state.current_temp_celsius -= 0.5 * delta_seconds
                self.state.battery_level_pct -= 0.001 * delta_seconds  # Background drain

            # Clamp values
            self.state.current_temp_celsius = max(20.0, min(95.0, self.state.current_temp_celsius))
            self.state.battery_level_pct = max(0.0, min(100.0, self.state.battery_level_pct))
    ...
    def _enforce_safety_limits(self):
        if self.state.battery_level_pct < 20.0:
            ...
            if self.state.battery_level_pct > 30.0 and self.state.current_temp_celsius < (self.specs.max_temp_celsius - 10):
    ...
    def _handle_message(self, msg: Dict, now: float) -> Optional[Dict]:
        ...
        if msg_type == "TASK_BID_REQUEST":
            ...
            if self.state.state == DeviceState.IDLE:
                ...
                return {"type": "ACK", "from": self.specs.device_id, "task_id": msg["task"]["id"]}
    ...
    def compute_tick(self, delta: float) -> Optional[Dict]:
        ...
        if new_progress >= 1.0:
            ...
            result = {
                ...
                "result_hash": hashlib.sha3_256(f"{self.active_task['id']}-{time.time()}".encode()).hexdigest(),
                "success": True
            }
            self.active_task = None
            return result
...
class ChaosOrchestrator:
    ...
    def add_chaos_event(self, event_type: ChaosEvent, target_ids: List[str] = None):
        ...
        if event_type == ChaosEvent.NODE_CRASH:
            ...
            if len(self.devices) > 0:
                ...
                logger.warning(f"CHAOS: Node {target} turned malicious (lying about power).")
    ...
    def print_summary(self):
        ...
        offline = sum(1 for d in self.devices.values() if d.state.state == DeviceState.OFFLINE)
        ...
        print(f"Network Health: {'CRITICAL' if failed > total_tasks * 0.2 else 'STABLE'}")
        print("="*40 + "\n")
...
class ScenarioFactory:
    ...
    def create_mixed_reality_network(count: int = 20) -> List[VirtualDevice]:
        ...
        devices = []
        for i in range(count):
            role = random.choices(["phone", "server", "malicious"], weights=[0.7, 0.2, 0.1])[0]
            
            if role == "phone":
                specs = HardwareSpecs(
                    device_id=f"phone_{i}",
                    cpu_cores=6, ram_gb=6, battery_capacity_mah=3500,
                    max_temp_celsius=75.0, is_tee_enabled=False,
                    network_bandwidth_mbps=50.0, honesty_factor=1.0
                )
            elif role == "server":
                specs = HardwareSpecs(
                    device_id=f"server_{i}",
                    cpu_cores=32, ram_gb=64, battery_capacity_mah=100000, # Plugged in
                    max_temp_celsius=95.0, is_tee_enabled=True,
                    network_bandwidth_mbps=1000.0, honesty_factor=1.0
                )
            else: # Malicious
                specs = HardwareSpecs(
                    device_id=f"liar_{i}",
                    cpu_cores=6, ram_gb=6, battery_capacity_mah=3500,
                    max_temp_celsius=75.0, is_tee_enabled=False,
                    network_bandwidth_mbps=50.0, honesty_factor=0.1 # Lies about power
                )
