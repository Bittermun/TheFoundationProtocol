...
def scenario_1_steady_state():
    ...
    config = ChaosConfig(probability_crash=0.0, malicious_node_ratio=0.0)
...
def scenario_5_mixed_reality():
    ...
    config = ChaosConfig(
        probability_crash=0.03, probability_latency=0.0, malicious_node_ratio=0.1
    )
    ...
    for device in orchestrator.devices.values():
        if device.specs.device_id.startswith("server"):
            ...
            task = {
                "id": f"heavy_{device.specs.device_id}",
                "reward": 100,
                "progress": 0.0,
            }
            device.active_task = task
            ...
            if random.random() > 0.3:
                task = {
                    "id": f"light_{device.specs.device_id}",
                    "reward": 25,
                    "progress": 0.0,
                }
                device.active_task = task
