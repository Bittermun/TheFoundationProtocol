...
class UITestRunner:
    ...
    async def test_onboarding(self) -> bool:
        ...
        try:
            ...
            print(f"   → Initialized with {len(self.adapter._content_cache)} cached items")
            print(f"   → Network: {status['broadcast_source']}, {status['neighbors_count']} neighbors")
    ...
    async def test_browse_content(self) -> bool:
        ...
        print(f"   → Browsed {len(items)} items, {len(emergency_items)} emergency alerts")
    ...
    async def test_share(self) -> bool:
        ...
        print(f"   → Shared voice recording")
        print(f"   → Earned {thanks} thanks")
        print(f"   → Broadcast to neighbors (simulated)")
    ...
    async def test_thanks_view(self) -> bool:
        ...
        print(f"   → Thanks summary retrieved")
    ...
    async def test_offline_handling(self) -> bool:
        ...
        required_keys = ['connected', 'broadcast_source', 'neighbors_count', 'cache_hit_rate']
        for key in required_keys:
            if key not in status:
                return False
        
        print(f"   → Offline handling: OK")
