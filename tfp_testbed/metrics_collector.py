...
class MetricsCollector:
    ...
    def record_bandwidth_savings(self, original_size: float, compressed_size: float, 
                                  node_id: str, region: str) -> None:
        """
    ...
    def record_reconstruction_time(self, content_hash: str, reconstruction_ms: float,
                                    node_id: str, region: str, network_type: str) -> None:
    ...
    def record_node_churn(self, active_nodes: int, total_nodes: int, 
                          churn_rate: float, node_id: str, region: str) -> None:
        """
            ...
            with open(self.metrics_file) as f:
                for line in f:
                    try:
                        ...
                        metric_time = datetime.fromisoformat(data['timestamp'].replace('Z', '+00:00'))
    ...
    def generate_testbed_report(self) -> Dict[str, Any]:
        ...
        for criterion, target in success_criteria.items():
            if criterion in aggregated["statistics"]:
                ...
                achieved = actual >= target if "savings" in criterion or "availability" in criterion else actual <= target
        ...
        report = {
            ...
            "overall_success_rate": sum(1 for a in achievements.values() if a["achieved"]) / len(achievements) * 100 if achievements else 0,
    ...
    def _generate_recommendations(self, aggregated: Dict, achievements: Dict) -> List[str]:
        ...
        if "bandwidth_savings" in aggregated["statistics"]:
            ...
            if savings < 50:
                recommendations.append("Consider increasing chunk cache size to improve bandwidth savings")
            elif savings > 80:
                recommendations.append("Excellent bandwidth efficiency; document case study for NGOs")
        ...
        if "reconstruction_time" in aggregated["statistics"]:
            ...
            if recon_time > 5000:  # >5 seconds
                recommendations.append("Optimize RaptorQ shard decoding or increase parallelization")
            elif recon_time < 1000:  # <1 second
                recommendations.append("Fast reconstruction achieved; suitable for real-time applications")
        
        # Check availability
        if "node_availability" in aggregated["statistics"]:
            availability = aggregated["statistics"]["node_availability"]["mean"]
            if availability < 70:
                recommendations.append("Increase node redundancy or implement better churn handling")
            elif availability > 90:
                recommendations.append("High availability achieved; ready for production deployment")
    ...
    def save_report(self, filepath: str = "TESTBED_REPORT.json") -> None:
        ...
        if report["success_achievements"]:
            ...
            for criterion, achievement in report["success_achievements"].items():
                ...
                print(f"  {status} {criterion}: {achievement['actual']:.1f} (target: {achievement['target']})")
...
def main():
    ...
    config = TestbedConfig(
        ...
        duration_days=30,
        target_metrics=["bandwidth_savings", "reconstruction_time", "node_availability"],
        success_criteria={
