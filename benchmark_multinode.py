...
def main():
    ...
    result = api_call(nodes[0], "POST", "/api/enroll", {
        "device_id": device_id,
    ...
    for label, size in test_sizes:
        ...
        result = api_call(nodes[0], "POST", "/api/publish", {
            "device_id": device_id,
            "title": label,
            "text": data,
        ...
        if content_hash == "unknown":
            log(f"✗ Publish returned invalid hash, skipping")
            continue
        ...
        earn_result = api_call(nodes[0], "POST", "/api/earn", {
            "device_id": device_id,
            "task_id": task_id
        ...
        for i, node in enumerate(nodes, 1):
            start = time.perf_counter()
            result = api_call(node, "GET", f"/api/get/{content_hash}?device_id={device_id}")
    ...
    for label, metrics in results.items():
        ...
        print(f"  Retrieve (avg across {metrics['nodes_successful']} nodes): {metrics['avg_retrieve_ms']:.0f}ms")
    ...
    if results:
        avg_retrieve_all = statistics.mean([m["avg_retrieve_ms"] for m in results.values()])
        print(f"Average retrieval latency: {avg_retrieve_all:.0f}ms")
        print(f"Nodes successfully serving content: {results[list(results.keys())[0]]['nodes_successful']}/10")
